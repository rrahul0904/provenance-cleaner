...
add(
  "Stripe TEST subscription env catalog",
  subscriptionPriceNames.every((name) => /^price_/u.test(process.env[name]?.trim() ?? "")),
  "three recurring price IDs required",
);
...
add(
  "Stripe TEST subscription fallback catalog",
  ["price_1UBmgJRB8OGmEnBwoUtRmBKb","price_1UBmhIRB8OGmEnBwFEJNV4zF","price_1UBmiARB8OGmEnBwgoMzZqbw"].every((id) => subscriptionCatalog.includes(id)),
  "controlled TEST fallback catalog committed",
);
...
add(
  ...
  Boolean(ownerBootstrapValid),
  ownerId ? "UUID configured" : ownerEmail ? "verified-email bootstrap configured" : "missing",
);
