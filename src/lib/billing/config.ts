...
const PREVIEW_PRICE_IDS: Record<CreditPackId, string> = {
...
export function getServerCreditPack(packId: CreditPackId) {
  ...
  const priceId = process.env[PRICE_ENV[packId]] || (process.env.VERCEL_ENV === "preview" ? PREVIEW_PRICE_IDS[packId] : undefined);
