...
export function readinessChecks(): Record<string, ReadinessCheck> {
  ...
  return {
    ...
    aiGateway: { configured: present("AI_GATEWAY_API_KEY") || present("VERCEL_OIDC_TOKEN"), required: true },
    turnstile: { configured: turnstileConfigured, required: true },
    rateLimitSalt: { configured: present("RATE_LIMIT_HASH_SALT") || present("VERCEL_OIDC_TOKEN"), required: true },
