...
- When the OAuth inlined-file API cannot accept the full tree in one orchestration, package build files into `release/oauth-bundle-*.json` transport parts. Every part must carry the certified `sourceHash`; `release/oauth-unpack.mjs` rejects mismatches and path traversal before the Next.js build.
