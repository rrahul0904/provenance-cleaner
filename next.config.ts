...
const nextConfig: NextConfig = {
  ...
  // Vercel's deployment environment is safe public build metadata. Expose it
  // explicitly so the browser can select the Preview-only Turnstile test key;
  // server-only secrets remain unavailable to client bundles.
