...
The email path never grants by an unverified address and is only a bootstrap mechanism. Once an enabled owner exists in the private `ops.admin_users` table, both bootstrap variables may be removed; runtime readiness then trusts the authoritative database owner state. Additional admin rows remain database-managed. Roles are `owner`, `admin`, and read-only `viewer`.
