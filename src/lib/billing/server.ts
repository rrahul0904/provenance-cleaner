...
async function rpc<T = JsonObject>(name: string, args: JsonObject): Promise<T> { const supabase = createAdminClient(); const { data, error } = await supabase.rpc(name, args); if (error) throwBillingError(error.message); return data as T; }
...
export async function getCreditBalance(userId: string) { return balanceFrom(await rpc<JsonObject>("billing_get_balance", { p_user_id: userId })); }
export async function reserveCredits(userId: string, operationKey: string, credits: number): Promise<CreditReservation> { const limits = getBillingLimits(); const data = await rpc<JsonObject>("billing_reserve_credits", { p_user_id: userId, p_operation_key: operationKey, p_credits: credits, p_requests_per_minute: limits.requestsPerMinute, p_credits_per_24h: limits.creditsPer24h, p_ttl_minutes: limits.reservationTtlMinutes }); return { reservationId: String(data.reservation_id), status: String(data.status) as CreditReservation["status"], credits: numeric(data.credits), created: Boolean(data.created), balance: balanceFrom(data) }; }
export async function commitReservation(userId: string, reservationId: string) { return balanceFrom(await rpc<JsonObject>("billing_commit_reservation", { p_user_id: userId, p_reservation_id: reservationId })); }
...
export async function expireStaleReservations(limit = 200) { return rpc<JsonObject>("billing_expire_stale_reservations", { p_limit: Math.max(1, Math.min(500, limit)) }); }
