...
const billing=vi.hoisted(()=>({
  ...
  grantSubscriptionInvoice:vi.fn(),
  linkStripeCustomer:vi.fn(),
  recordCheckoutAmount:vi.fn(),
  recordPolicyRefund:vi.fn(),
  recordSubscriptionInvoiceAmount:vi.fn(),
  upsertSubscription:vi.fn(),
  getRefundQuote:vi.fn(),
  recordPurchaseRefund:vi.fn(),
}));
const identity=vi.hoisted(()=>({getRequestIdentity:vi.fn()}));
const stripeFns=vi.hoisted(()=>({constructEvent:vi.fn(),retrieveSession:vi.fn(),createRefund:vi.fn()}));
...
vi.mock("../src/lib/billing/stripe",()=>({getStripe:()=>({webhooks:{constructEvent:stripeFns.constructEvent},checkout:{sessions:{retrieve:stripeFns.retrieveSession}},refunds:{create:stripeFns.createRefund}})}));
...
function checkoutEvent(country:string|null,id="evt_test_1"){
  return {id,type:"checkout.session.completed",data:{object:{id:"cs_test_policy",mode:"payment",payment_status:"paid",amount_total:499,currency:"usd",payment_intent:"pi_test_policy",metadata:{purchase_id:PURCHASE_ID},client_reference_id:PURCHASE_ID,collected_information:{shipping_details:{address:{country}}},customer_details:{address:{country}}}}};
}
async function webhookPost(){
  const {POST}=await import("../src/app/api/billing/webhook/route");
  return POST(new Request("http://localhost/api/billing/webhook",{method:"POST",headers:{"stripe-signature":"sig_test","content-type":"application/json"},body:"{}"}));
}
async function refundPost(purchaseId=PURCHASE_ID){
  const {POST}=await import("../src/app/api/account/refund/route");
  return POST(new Request("http://localhost/api/account/refund",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({purchaseId})}));
}
...
describe("Stripe TEST reconciliation routes",()=>{
  beforeEach(()=>{
    ...
    billing.recordCheckoutAmount.mockResolvedValue({recorded:true});
    ...
    billing.recordSubscriptionInvoiceAmount.mockResolvedValue({recorded:true});
  });

  afterEach(()=>{process.env.STRIPE_WEBHOOK_SECRET=originalWebhookSecret;vi.resetModules();});

  it("records privacy-safe amount evidence for an accepted US purchase",async()=>{
    stripeFns.constructEvent.mockReturnValue(checkoutEvent("US"));
    const response=await webhookPost();
    expect(response.status).toBe(200);
    expect(billing.completeCheckoutPurchase).toHaveBeenCalledWith("evt_test_1","checkout.session.completed",PURCHASE_ID,"cs_test_policy");
    expect(billing.recordCheckoutAmount).toHaveBeenCalledWith({purchaseId:PURCHASE_ID,sessionId:"cs_test_policy",amount:499,currency:"usd"});
  });
  ...
  it("refunds a paid non-US Checkout instead of granting credits",async()=>{
    ...
    expect(stripeFns.createRefund).toHaveBeenCalledWith(expect.objectContaining({payment_intent:"pi_test_policy",amount:499,metadata:{purchase_id:PURCHASE_ID,policy_reason:"country_policy"}}),{idempotencyKey:"policy-refund:country_policy:cs_test_policy"});
  ...
  it("refunds a paid Checkout when billing reports that the account was deleted",async()=>{
    ...
    expect(response.status).toBe(200);
    expect(billing.recordCheckoutAmount).toHaveBeenCalledWith({purchaseId:PURCHASE_ID,sessionId:"cs_test_policy",amount:499,currency:"usd"});
    expect(stripeFns.createRefund).toHaveBeenCalledWith(expect.objectContaining({payment_intent:"pi_test_policy",amount:499,metadata:{purchase_id:PURCHASE_ID,policy_reason:"account_deleted"}}),{idempotencyKey:"policy-refund:account_deleted:cs_test_policy"});
  ...
  it("uses the same policy-refund idempotency key when a webhook is retried after DB reconciliation failure",async()=>{
    ...
    const first=await webhookPost();expect(first.status).toBe(500);
    const second=await webhookPost();expect(second.status).toBe(200);
    expect(stripeFns.createRefund).toHaveBeenCalledTimes(2);
    expect(stripeFns.createRefund.mock.calls[1][1]).toEqual(stripeFns.createRefund.mock.calls[0][1]);
  });
  ...
  it("creates a proportional refund for only the unused credits in one purchase lot",async()=>{
    ...
    expect(response.status).toBe(200);
    expect(stripeFns.createRefund).toHaveBeenCalledWith(expect.objectContaining({payment_intent:"pi_test_customer",amount:349,metadata:{purchase_id:PURCHASE_ID,credits_refunded:"7"}}),{idempotencyKey:`purchase-refund:${PURCHASE_ID}:7`});
