...
export function AccountBar() {
  ...
  const refreshIdentity = useCallback(async () => {
    ...
    setIdentity({ userId: claims.sub, isAnonymous: claims.is_anonymous === true });
  ...
  async function google() { if (!supabase) return; setBusy(true); setMessage(null); const redirectTo = `${window.location.origin}/auth/callback?next=/&promo=${identity?.isAnonymous ? "1" : "0"}`; const result = identity?.isAnonymous ? await supabase.auth.linkIdentity({ provider: "google", options: { redirectTo } }) : await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo } }); if (result.error) { setMessage("Google sign-in could not be started."); setBusy(false); } }
