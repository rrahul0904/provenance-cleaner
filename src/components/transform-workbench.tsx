...
const MODES: Array<{ id: TransformMode; label: string; description: string }> = [
  { id: "parity", label: "Parity", description: "Sanitize wording aggressively while holding meaning, protected facts, and approximately the same length." },
...
export function TransformWorkbench() {
  ...
  function invalidateResult() { revision.current += 1; activeRequest.current?.abort(); setResult(null); setError(null); }
  ...
  return <section className="transform-section" aria-label="Semantics-preserving editor">
    <div className="section-heading"><div><p className="eyebrow">Protected semantic editing</p><h2>Edit the prose. Keep the facts.</h2><p>Parity mode uses a tighter length contract, deterministic proper-name/fact protection, and limits consecutive carry-over outside protected spans. Each operation accepts up to {MAX_REWRITE_WORDS.toLocaleString()} words.</p></div><span className="pill">~{estimatedCredits} credit{estimatedCredits === 1 ? "" : "s"}</span></div>
    <div className="transform-grid"><div className="panel transform-input-panel">
      ...
      <p className="privacy-note">Text stays local until you explicitly run an edit. Raw source/result text is not intentionally persisted or written to operational logs.</p>{error && <div className="error-card" role="alert">{error}</div>}
    </div><div className="panel transform-output-panel" aria-live="polite">
      ...
      {!result ? <div className="empty-state compact">Run an edit to see the revised prose, deterministic receipt, and committed credit charge.</div> : <><div className="transform-metrics"><Metric label="Protected" value={`${result.metrics.protectedPreserved}/${result.metrics.protectedTotal}`} /><Metric label="Length retained" value={`${Math.round(result.receipt.retainedPercent)}%`} /><Metric label="Wording replaced" value={`${Math.round(result.receipt.wordingReplacedPercent)}%`} /><Metric label="Longest unprotected" value={`${result.receipt.longestUnprotectedSharedWordRun} words`} /></div><div className="clean-output transform-output"><div className="output-heading"><strong>Revised prose</strong><button className="copy" onClick={copyResult}>Copy</button></div><pre>{result.text}</pre><p>{result.model} · {result.attempts} attempt{result.attempts === 1 ? "" : "s"} · {result.metrics.sourceWords} → {result.metrics.outputWords} words</p></div><div className="verification-box"><strong>Deterministic receipt</strong><div className="verification-stats"><span>facts/entities {result.receipt.checks.numericDateEntityPreserved}/{result.receipt.checks.numericDateEntityExpected}</span><span>quotes/refs {result.receipt.checks.quoteReferencePreserved}/{result.receipt.checks.quoteReferenceExpected}</span><span>{result.receipt.protectedSpanCount} protected spans</span></div><p>Text-watermark verifier: {result.watermark.available ? result.watermark.status : "unavailable"}. {result.watermark.note}</p></div>{result.billing && <div className="verification-box"><strong>Billing receipt</strong><div className="verification-stats"><span>{result.billing.creditsCharged} credit{result.billing.creditsCharged === 1 ? "" : "s"} charged</span><span>{result.billing.balanceAfter} remaining</span><span>operation {result.billing.operationId.slice(0, 8)}</span></div><p>The debit was committed only after preservation validation passed.</p></div>}{result.warnings.length > 0 && <div className="notice-card"><strong>Review note</strong><ul>{result.warnings.map(w => <li key={w}>{w}</li>)}</ul></div>}<div className="verification-box"><strong>What “validated” means here</strong><p>Protected factual spans survived exactly and deterministic checks passed. It does not prove human authorship, detector performance, or statistical watermark removal.</p></div></>}
    </div></div>
  </section>;
}
